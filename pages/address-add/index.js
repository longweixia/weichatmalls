...
Page({
  ...
  bindSave: function (e) {
    ...
    var code = e.detail.value.code;   //邮政编码
    ...
    // commonCityData.cityData所有城市信息的集合
    ...
    wx.request({
      ...
      data: {
        ...
        id: apiAddid,  //用户id
        provinceId: commonCityData.cityData[this.data.selProvinceIndex].id,//所属省份编码
        cityId: cityId,
...
// 初始化城市数据
  initCityData: function (level, obj) {
    ...
    if (level == 1) {
      ...
      var pinkArray = [];
      var dataArray = obj.cityList  // 城市列表
      ...
      var pinkArray = [];
      var dataArray = obj.districtList//所属区县列表
      for (var i = 0; i < dataArray.length; i++) {
        // // pinkArray每个辖区名字的集合
        pinkArray.push(dataArray[i].name);
      }
  ...
  bindPickerProvinceChange: function (event) {
    // commonCityData.cityData所有地址数据  event.detail.value当前选中的哪个省份  触发input事件，event.detail={value:value}
    var selIterm = commonCityData.cityData[event.detail.value]; //
    this.setData({
      selProvince: selIterm.name,//选中的省份
      selProvinceIndex: event.detail.value,//选中省份的位置 
      selCity: '请选择',
      selCityIndex: 0,
      selDistrict: '请选择',
      selDistrictIndex: 0
